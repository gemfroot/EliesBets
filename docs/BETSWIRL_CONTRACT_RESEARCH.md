...
### 5. Fuji Gas Price is Extremely Low (2 wei)

Avalanche Fuji's gas price is ~2 wei, not gwei. This breaks VRF cost estimation since `getChainlinkVRFCost` uses `tx.gasprice`. When the real gas price is 2 wei, the VRF cost comes out to essentially zero, but Chainlink still charges based on their own pricing.

**Workaround**: Use a minimum floor of 25 gwei when estimating VRF cost:
```javascript
const gasPrice = await publicClient.getGasPrice();
const effectiveGasPrice = gasPrice > 25000000000n ? gasPrice : 25000000000n;
```

The actual VRF cost on Fuji is ~0.016 AVAX. Send a 50% buffer above estimated cost.
...
- Bet 0.1 LINK on HEADS
- Win: payout = 0.1 × 2 × (1 - 0.04) = 0.192 LINK
- Lose: payout = 0
...
**Status: Deployed and configured. VRF subscription created. Remaining: set VRF sub ID on contract, add consumer, update frontend, test bet.**
